"use strict";
exports.__esModule = true;
/**
 * Find the intersection point between two lines located on the lines.
 * @name intersect
 * @param {Array<number>} l1 A nested array containing x and y coordinates. [[0, 0], [2, 2]]
 * @param {Array<number>} l2 A nested array containing x and y coordinates. [[1, 2], [3, 1]]
 * @returns {(Array<number>|Boolean)} An array containing the intersection.
 * False if no intersection.
 * @example
 * const lineSegment1 = [[0, 0, 0], [2, 2, 0]];
 * const lineSegment2 = [[1, 2, 0], [3, 1, 0]];
 * console.log(intersect(lineSegment1, lineSegment2));
 * // output: [1.6666666666666667, 1.6666666666666667]
 */
function intersect(l1, l2) {
    if ( // Check if the two segments are equal.
    l1[0][0] === l2[0][0] && l1[0][1] === l2[0][1] &&
        l1[1][0] === l2[1][0] && l1[1][1] === l2[1][1]) {
        return [l1[0][0], l1[0][1]]; // return first point
    }
    if ( // Check if they intersect at first end.
    (l1[0][0] === l2[0][0] && l1[0][1] === l2[0][1]) ||
        (l1[0][0] === l2[1][0] && l1[0][1] === l2[1][1])) {
        return [l1[0][0], l1[0][1]]; // return first point
    }
    if ( // Check if they intersect at second end.
    (l1[1][0] === l2[0][0] && l1[1][1] === l2[0][1]) ||
        (l1[1][0] === l2[1][0] && l1[1][1] === l2[1][1])) {
        return [l1[1][0], l1[1][1]]; // return first point
    }
    var dx1 = l1[1][0] - l1[0][0];
    var dy1 = l1[1][1] - l1[0][1];
    var dx2 = l2[1][0] - l2[0][0];
    var dy2 = l2[1][1] - l2[0][1];
    var denom = (dy2 * dx1) - (dx2 * dy1);
    if (denom === 0) {
        return false;
    }
    var dx12 = l1[0][0] - l2[0][0];
    var dy12 = l1[0][1] - l2[0][1];
    var ua = ((dx2 * dy12) - (dy2 * dx12)) / denom;
    var ub = ((dx1 * dy12) - (dy1 * dx12)) / denom;
    if (ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1) {
        var ip = [l1[0][0] + (ua * dx1), l1[0][1] + (ua * dy1)];
        return ip;
    }
    return false;
}
exports.intersect = intersect;
/**
 * Finds the distance between two points.
 * @name distance
 * @param {Array<number>} p1 An array containing x and y coordinates. Such as: [0, 0]
 * @param {Array<number>} p2 An array containing x and y coordinates. Such as: [2, 3]
 * @returns {number} The distance between the points in map units.
 * @example
 * const point1 = [0, 0];
 * const point2 = [2, 3];
 * console.log(distanceTo(point1, point2));
 * // output: 3.7416573867739413
 */
function distance(p1, p2) {
    return Math.sqrt((Math.pow((p2[0] - p1[0]), 2)) + (Math.pow((p2[1] - p1[1]), 2)));
}
exports.distance = distance;
/**
 * Finds the angle between two points. 0-360 system increasing clockwise with north being 0.
 * Returns zero in the case of identical points.
 * @name angle
 * @param {Array<number>} p1 An array containing x and y coordinates. Such as: [0, 0]
 * @param {Array<number>} p2 An array containing x and y coordinates. Such as: [2, 2]
 * @returns {number} Angle between points in radians.
 * @example
 * const point1 = [0, 0];
 * const point2 = [2, 2];
 * console.log(angle(point1, point2));
 * // output: 45 // now radians
 */
function angle(p1, p2) {
    // If the two points are identical
    if (p1[0] === p2[0] && p1[1] === p2[1]) {
        return 0;
    }
    var theta = Math.atan2(p1[0] - p2[0], p2[1] - p1[1]);
    if (theta < 0) {
        theta += Math.PI * 2;
    }
    return theta;
}
exports.angle = angle;
/**
 * Tests if a given point is located on a given line. Only works in 2D.
 * @name isPointOnLine
 * @param {Array<number>} p An array containing x and y coordinates. Such as: [1,1]
 * @param {Array<number>} l A nested array containing x and y coordinates. Such as: [[1,1], [2,2]]
 * @param {Number} [t = Number.EPSILON] The tolerance of the distance between the point and the line. Defaults to Epsilon.
 * @returns {Boolean} True if point is on line within tolerance, false otherwise.
 * @example
 * const point = [1, 1];
 * const line = [[0, 0], [2, 2]];
 * console.log(isPointOnLine(point, line));
 * // output: true
 */
function isPointOnLine(p, l, t) {
    if (t === void 0) { t = Number.EPSILON; }
    var dy = l[1][1] - l[0][1];
    var dx = l[1][0] - l[0][0];
    if (dx === 0 || dy === 0) { // vertical line
        if (p[0] === l[0][0]) {
            var ymin = void 0;
            var ymax = void 0;
            if (l[0][1] >= l[1][1]) {
                ymin = l[1][1];
                ymax = l[0][1];
            }
            else {
                ymin = l[0][1];
                ymax = l[1][1];
            }
            if (p[1] >= ymin && p[1] <= ymax) {
                return true;
            }
        }
        else if (p[1] === l[0][1]) { // horisontal line
            var xmin = void 0;
            var xmax = void 0;
            if (l[0][0] >= l[1][0]) {
                xmin = l[1][0];
                xmax = l[0][0];
            }
            else {
                xmin = l[0][0];
                xmax = l[1][0];
            }
            if (p[0] >= xmin && p[0] <= xmax) {
                return true;
            }
        }
        return false;
    }
    var slope = dy / dx;
    var intercept = l[0][1] - (slope * l[0][0]);
    var on = Math.abs(p[1] - ((slope * p[0]) + intercept));
    return (on <= t) === true;
}
exports.isPointOnLine = isPointOnLine;
/**
 * Creates a new point from the angle between two points and a provided distance from the first point.
 * @name pointFromTwoPoints
 * @param {Array<number>} p1 An array containing x and y coordinates. Such as: [0, 0]
 * @param {Array<number>} p2 An array containing x and y coordinates. Such as: [2, 2]
 * @param {number} [d = 0] The distance to the new point. Can be higher than the distance between the two points.
 * @returns {Array<number>} An array containing the x and y coordinates of the new point.
 * @example
 * const point1 = [0, 0];
 * const point2 = [2, 2];
 * console.log(pointFromTwoPoints(point1, point2, Math.sqrt(3)));
 * // output: [1, 1]
 */
function pointFromTwoPoints(p1, p2, d) {
    if (d === void 0) { d = 0; }
    if (d === 0) {
        return p1;
    }
    var dx = p2[0] - p1[0];
    var dy = p2[1] - p1[1];
    var xy = (Math.pow(dx, 2)) + (Math.pow(dy, 2));
    var m = Math.sqrt(xy);
    return [
        p1[0] + ((dx / m) * d),
        p1[1] + ((dy / m) * d),
    ];
}
exports.pointFromTwoPoints = pointFromTwoPoints;
/**
 * Finds a point at the ratio between two points.
 * @name pointFromRatio
 * @param {Array<number>} p1 An array containing x and y coordinates. Such as: [0, 0]
 * @param {Array<number>} p2 An array containing x and y coordinates. Such as: [2, 2]
 * @param {number} r A number between 0 - 1;
 * @returns {Array} A point located at the given ratio between the two points.
 * @example
 * const point1 = [0, 0];
 * const point2 = [2, 2];
 * console.log(pointFromRatio(point1, point2, 0.5));
 * // output: [1, 1]
 */
function pointFromRatio(p1, p2, r) {
    if (r === void 0) { r = 0.5; }
    if (r === 0) {
        return p1;
    }
    if (r === 1) {
        return p2;
    }
    var dx = p2[0] - p1[0];
    var dy = p2[1] - p1[1];
    return [
        p1[0] + (dx * r),
        p1[1] + (dy * r),
    ];
}
exports.pointFromRatio = pointFromRatio;
/**
 * Creates a new point given a point an angle and a distance.
 * 0-360 system increasing clockwise with north being 0.
 * @name pointFromAngle
 * @param {Array<number>} p An array containing x and y coordinates.
 * @param {number} a A number representing an angle in radians.
 * @param {number} [d = 0] The distance to the new point.
 * @returns {Array} An Array containing x, y and potentially z coordinates.
 * @example
 * const point = [0, 0];
 * const angle = Math.PI / 4;
 * const distance = Math.sqrt(2);
 * console.log(pointFromAngle(point, angle, distance));
 * // output: [1, 1]
 */
function pointFromAngle(p, a, d) {
    if (d === void 0) { d = 0; }
    return [
        (Math.cos(a) * d) + p[0],
        (Math.sin(a) * d) + p[1],
    ];
}
exports.pointFromAngle = pointFromAngle;
/**
 * Rotate a point around a custom origin.
 * @name pointFromOriginRotation
 * @param {Array<number>} o An array containing x and y coordinates. Such as: [0, 0]
 * @param {Array<number>} p An array containing x and y coordinates. Such as: [0, 2]
 * @param {number} a A number representing an angle in radians.
 * @returns {Array<number>} A new point rotate according to the provided angle around origin.
 * @example
 * const origin = [0, 0];
 * const point = [0, 2];
 * const angle = Math.PI / 2;
 * console.log(pointRotation(origin, point, angle));
 * // output: [2, 0]
 */
function pointFromOriginRotation(o, p, a) {
    var cos = Math.cos(a);
    var sin = Math.sin(a);
    var nx = ((cos * (p[0] - o[0])) +
        (sin * (p[1] - o[1]))) + o[0];
    var ny = ((cos * (p[1] - p[1])) -
        (sin * (p[0] - p[0]))) + o[1];
    return [nx, ny];
}
exports.pointFromOriginRotation = pointFromOriginRotation;
/**
 * Finds the nearest point on a lineSegment from a point.
 * @name nearestPointOnLine
 * @param {Array<number>} p An array containing x and y coordinates. Such as: [1, 1]
 * @param {Array<number>} l An nested array containing x and y coordinates. Such as: [[0, 0], [2, 2]]
 * @returns {Array<number>} The nearest point on the lineSegment.
 * @example
 * const point = [1, 1];
 * const lineSegment = [[0, 0], [2, 2]];
 * console.log(nearestPointOnLine(point, lineSegment);
 * // output: [2.5, 2, 0.5]
 */
function nearestPointOnLine(p, l) {
    var dlx = l[1][0] - l[0][0];
    var dly = l[1][1] - l[0][1];
    var dpx = p[0] - l[0][0];
    var dpy = p[1] - l[0][1];
    var c1 = (dlx * dpx) + (dly * dpy);
    if (c1 <= 0) {
        return l[0];
    }
    var c2 = (Math.pow(dlx, 2)) + (Math.pow(dly, 2));
    if (c2 <= c1) {
        return l[1];
    }
    var b = c1 / c2;
    var s = [dlx * b, dly * b];
    return [l[0][0] + s[0], l[0][1] + s[1]];
}
exports.nearestPointOnLine = nearestPointOnLine;
/**
 * Calculates the area of a simple polygon.
 * @name area
 * @param {Array<number>} poly A nested array containing x and y coordinates.
 * @returns {number} The area of the polygon.
 * @example
 * const polygon = [[0,0], [0,2], [2,2], [2,0], [0,0]];
 * console.log(area(polygon));
 * // output: 4
 */
function area(poly) {
    var total = 0;
    for (var i = 0; i < poly.length - 1; i += 1) {
        var j = (i === poly.length - 1) ? 0 : i + 1;
        total += (poly[i][0] * poly[j][1]) - (poly[j][0] * poly[i][1]);
    }
    return Math.abs(total) / 2;
}
exports.area = area;
/**
 * Calculates the bounding box of a polygon.
 * @name bbox
 * @param {Array<number>} polygon A nested array containing x and y coordinates.
 * @returns {Array<number>} A nested array containing the bounding box. As: [Xmin, Ymin, Xmax, Ymax]
 * @example
 * const polygon = [[0,0], [0,3], [2,2], [1,0], [0,0]];
 * console.log(bbox(polygon));
 * // output: [0, 0, 2, 3]
 */
function bbox(poly) {
    var b = [Infinity, Infinity, -Infinity, -Infinity];
    for (var i = 0; i < poly.length - 1; i += 1) {
        if (b[0] > poly[i][0]) {
            b[0] = poly[i][0];
        }
        if (b[1] > poly[i][1]) {
            b[1] = poly[i][1];
        }
        if (b[2] < poly[i][0]) {
            b[2] = poly[i][0];
        }
        if (b[3] < poly[i][1]) {
            b[3] = poly[i][1];
        }
    }
    return b;
}
exports.bbox = bbox;
/**
 * Calculates the length of a linestring.
 * @name lineStringLength
 * @param {Array<number>} ls A nested array containing x and y coordinates.
 * @returns {number} The distance in the same units as the input.
 * @example
 * const polygon = [[0,0], [0,3], [2,2], [1,0]];
 * console.log(lineStringLength(polygon));
 * // output: length // TODO
 */
function lineStringLength(ls) {
    var length = 0;
    for (var i = 0; i < ls.length - 1; i += 1) {
        length += distance(ls[i], ls[i + 1]);
    }
    return length;
}
exports.lineStringLength = lineStringLength;
/**
 * Creates a new point at a supplied distance along a linestring.
 * Extrapolates from the last two points if the distance supplied is longer than the length of the linestring.
 * @name pointAlongLineString
 * @param {Array<number>} ls A nested array containing x and y coordinates.
 * @param {number} [d = 0] The distance to the new point.
 * @returns {Array<number>} A new point at the supplied distance along the linestring.
 * @example
 * const polygon = [[0,0], [0,3], [2,2], [1,0]];
 * console.log(lineStringLength(polygon));
 * // output: length // TODO
 */
function pointAlongLineString(ls, d) {
    if (d === void 0) { d = 0; }
    if (d === 0) {
        return ls[0];
    }
    var a = 0;
    for (var i = 0; i < ls.length - 1; i += 1) {
        a += distance(ls[i], ls[i + 1]);
        if (a === d) {
            return ls[i + 1];
        }
        if (a > d) {
            return pointFromTwoPoints(ls[i + 1], ls[i], a - d);
        }
    }
    return pointFromTwoPoints(ls[ls.length - 2], ls[ls.length - 1]);
}
exports.pointAlongLineString = pointAlongLineString;
function perpendicularPoints(line, lineDistance, distance) {
    var origin = pointFromTwoPoints(line[0], line[1], lineDistance);
    var left = pointFromAngle(origin, distance, angle(line[0], line[1]) - (0.5 * Math.PI));
    var right = [
        left[0] + ((-1) * (origin[0] - left[0])),
        left[1] + ((-1) * (origin[1] - left[1])),
    ];
    return [left, right];
}
exports.perpendicularPoints = perpendicularPoints;
function shiftLines(line, distance) {
    var ls = pointFromAngle(line[0], distance, angle(line[0], line[1]) - (0.5 * Math.PI));
    var tv = [line[0][0] - ls[0], line[0][1] - ls[1]];
    var tvi = [tv[0] * -1, tv[1] * -1];
    var le = [line[1][0] + tv[0], line[1][1] + tv[1]];
    var rs = [ls[0] + tvi[0], ls[1] + tvi[1]];
    var re = [le[0] + tvi[0], le[1] + tvi[1]];
    return [
        [ls, le],
        [rs, re],
    ];
}
exports.shiftLines = shiftLines;
function shiftLeft(line, distance) {
    var ls = pointFromAngle(line[0], distance, angle(line[0], line[1]) - (0.5 * Math.PI));
    var tv = [line[0][0] - ls[0], line[0][1] - ls[1]];
    var le = [line[1][0] + tv[0], line[1][1] + tv[1]];
    return [ls, le];
}
exports.shiftLeft = shiftLeft;
function shiftRight(line, distance) {
    var rs = pointFromAngle(line[0], distance, angle(line[0], line[1]) + (0.5 * Math.PI));
    var tv = [line[0][0] - rs[0], line[0][1] - rs[1]];
    var re = [line[1][0] + tv[0], line[1][1] + tv[1]];
    return [rs, re];
}
exports.shiftRight = shiftRight;
function isPointInsidePolygon(point, polygon, precision) {
    var x = point[0];
    var y = point[1];
    for (var i = 0; i < polygon.length; i += 1) {
        var j = (i > 0) ? i - 1 : polygon.length - 1;
        var xi = polygon[i][0];
        var yi = polygon[i][1];
        var xj = polygon[j][0];
        var yj = polygon[j][1];
        if (point[0] === polygon[i][0] && point[1] === polygon[i][1]) {
            return true;
        }
        var isIntersecting = ((yi > y) !== (yj > y)) &&
            ((x < (xj - xi) * (y - yi)) / ((yj - yi) + xi));
        if (isIntersecting) {
            return true;
        }
        if (precision &&
            polygon[i + 1] &&
            isPointOnLine([polygon[i], polygon[i + 1]], point) < precision // TODO: CHECK
        ) {
            return true;
        }
    }
    return false;
}
exports.isPointInsidePolygon = isPointInsidePolygon;
function createSectorSlice(point, b1, b2, distance, precision) {
    if (precision === void 0) { precision = 24; }
    var ad = Math.min((2 * Math.PI) - Math.abs(b1 - b2), Math.abs(b1 - b2));
    var nodes = Math.ceil(ad / (Math.PI * 2) / precision);
    var rads = ad / (nodes + 1);
    var cl = nodes + 2;
    var coords = Array(cl);
    coords[0] = [
        (Math.cos(b1) * distance) + point[0],
        (Math.sin(b1) * distance) + point[1],
    ];
    coords[cl - 1] = [
        (Math.cos(b2) * distance) + point[0],
        (Math.sin(b2) * distance) + point[1],
    ];
    for (var i = 1; i + 1 < cl; i += 1) {
        var a = b1 + (rads * i);
        coords[i] = [
            (Math.cos(a) * distance) + point[0],
            (Math.sin(a) * distance) + point[1],
        ];
    }
    return coords;
}
exports.createSectorSlice = createSectorSlice;
function createSector(point, b1, b2, distance, precision) {
    if (precision === void 0) { precision = 24; }
    var bx = Math.abs(b1 - b2);
    var ad = Math.min((2 * Math.PI) - bx, bx);
    var nodes = Math.ceil(ad / (Math.PI * 2) / precision);
    var rads = ad / (nodes + 1);
    var cl = nodes + 4;
    var coords = Array(cl);
    coords[0] = point;
    coords[1] = [
        (Math.cos(b1) * distance) + point[0],
        (Math.sin(b1) * distance) + point[1],
    ];
    coords[cl - 2] = [
        (Math.cos(b2) * distance) + point[0],
        (Math.sin(b2) * distance) + point[1],
    ];
    coords[cl - 1] = point;
    var t = 1;
    for (var i = 2; i + 2 < cl; i += 1) {
        var a = b1 + (rads * t);
        coords[i] = [
            (Math.cos(a) * distance) + point[0],
            (Math.sin(a) * distance) + point[1],
        ];
        t += 1;
    }
    return coords;
}
exports.createSector = createSector;
function createCircle(point, radius, precision) {
    if (precision === void 0) { precision = 24; }
    var coords = Array(precision + 1);
    for (var i = 0; i < precision; i += 1) {
        var angle_1 = (i / (precision / 2)) * Math.PI;
        var x = point[0] - (radius * Math.cos(angle_1));
        var y = point[1] - (radius * Math.sin(angle_1));
        coords[i] = [x, y];
    }
    coords[coords.length - 1] = point;
    return coords;
}
exports.createCircle = createCircle;
