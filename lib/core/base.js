 ...
 * // output: [2, 0]
 */
function pointFromOriginRotation(p, o = [0, 0], a) {
    const cos = Math.cos(a);
    const sin = Math.sin(a);
    const dx = p[0] - o[0];
    const dy = p[1] - o[1];
    const nx = (cos * dx) + (sin * dy) + o[0];
    const ny = (cos * dy) - (sin * dx) + o[1];
    return [nx, ny];
}
 ...
 * @returns {Array<number>} A new point at the supplied distance along the linestring.
 */
function perpendicularPointsSegment(l, ld, d, b = true) {
...
exports.perpendicularPointsSegment = perpendicularPointsSegment;
/**
 ...
 */
function isPointInsidePolygon(p, poly) {
    const x = p[0];
    const y = p[1];
 ...
 * @returns {boolean} returns true if inside, false if not.
 */
function createCircle(p, r, precision = 24) {
