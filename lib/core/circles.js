/* eslint-env node, es6  */
function createSectorSlice(point, b1, b2, distance, precision = 64) {
  const ad = Math.min(
...
function createSector(point, b1, b2, distance, precision = 64) {
  const ad = Math.min(
    (2 * Math.PI) - Math.abs(b1 - b2), Math.abs(b1 - b2));
  const nodes = Math.ceil(ad / (Math.PI * 2) / precision);
  const rads = ad / (nodes + 1);
...
function createCircle(point, radius, precision = 64) {
