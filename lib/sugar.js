...
base.lineSegmentLength = line => base.distancePoints(line[0], line[1]);

// LENGTH OF LINE (3D)
base.lineLength = (line) => {
  let length = 0;
  for (let i = 0; i < line.length - 1; i += 1) {
    length += base.distancePoints(line[0], line[0 + 1]);
  }
  return length;
};
...
base.pointAlongLine = (line, distance = 0) => {
  const segmentLength = base.lineSegmentLength(line);
...
base.perpendicularPoint = (line, lineDistance, radius, boolean) => {
  const angle = base.anglePoints(line[0], line[1]);
...
base.shiftLine = (line, radius, boolean) => {
  ...
  const end = base.perpendicularPoint(line, base.lineSegmentLength(line), radius);
