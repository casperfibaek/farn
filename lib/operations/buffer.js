...
const bufferLine = (line, distance, precision) => {
  ...
  for (let i = 0; i < line.length - 1; i += 1) {
    const currentShiftLines = base.shiftLines([line[i], line[i + 1]], distance);
    const currLeft = currentShiftLines[0];
    const currRight = currentShiftLines[1];

    if (i < line.length - 2) {
      if (i === 0) { left.push(currLeft[0]); }
      const forwardShiftLines = base.shiftLines([line[i + 1], line[i + 2]], distance);
    ...
    if (i === line.length - 2) {
      const shiftLines = base.shiftLines([line[i], line[i + 1]], distance);
